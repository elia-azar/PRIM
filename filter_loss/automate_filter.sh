...
do
	...
	for filter in "${FilterArray[@]}"; do
		...
		do
			...
			./dump_automate.sh -n $p -f $filter &
			sleep 4
			timeout 60s ./moongen.sh -p $p > test_generator.txt
			sleep 2
			echo "New results $j" >> results_generator${p}_${filter}.txt
			cat test_generator.txt | awk '/StdDev/ {print $0} {}' >> results_generator${p}_${filter}.txt
			sleep 8
			rm /dev/hugepages/*
		done
	done
