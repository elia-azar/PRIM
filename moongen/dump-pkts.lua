...
function dumper(queue, args, threadId)
	...
	print("This is the local filter: " .. args.filter)
	local snapLen = args.snapLen
	local writer
	local captureCtr, filterCtr
	if args.file then
		print("A pcap file has been defined")
		...
		rxCtr = stats:newPktRxCounter("RX packets, thread #" .. threadId)
	end
